...
class Game {
  ...
  constructor(width: number, height: number) {
    ...
    this.paddleHeight = 210; // Fixed paddle height
  ...
  checkWinner() {
    console.log(
      "user_id p1:",
      this.p1.user_id + " | user_id p2:",
      this.p2.user_id
    );
    console.log(
      "Score Player 1:",
      this.score_p1 + " | Score Player 2:",
      this.score_p2
    );
    if (this.score_p2 == 5) {
      console.log(this.p2.user_id);
      if (this.mode === "local") return "PLAYER_B";
      if (this.mode === "ia") return "Computer";
  ...
  checkPaddleCollision(paddle: Paddle) {
    ...
    if (
      ...
      this.ball.dx < 0 &&
      nextX - this.ball.radius < paddle.x + paddle.width &&
  ...
  async endGame(winnerUserId: string) {
    ...
    const winnerName =
      this.p1.user_id === winnerUserId ? this.p1.username : this.p2.username;
  ...
  async endGameWithDisconnection(
    winnerUserId: string,
    disconnectedPlayer: "p1" | "p2"
  ) {
    this.is_end = true;
    this.resetBall(); // Reset ball position and speed
    ...
    const gameFinishedMessage = {
      ...
      data: {
        ...
        disconnection: true,
        disconnected_player:
          disconnectedPlayer === "p1" ? this.p1.username : this.p2.username,
      },
    };
  ...
  sendData() {
    ...
    if (this.mode === "random_adversaire" && (!this.p1.ws || !this.p2.ws)) {
      if (this.p1.ws) {
        this.p1.ws.send(
          JSON.stringify({ type: "game_state", data: gameState })
        );
      }
      return;
    }
