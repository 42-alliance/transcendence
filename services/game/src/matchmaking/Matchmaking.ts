...
class QueueManager {
    ...
    while (this.pendingPlayers.length > 0) {
      ...
      if (queue) {
        if (!queue.some((p) => p.username === player.username)) {
          queue.push(player);
          console.log(
            `Player ${player.username} moved to ${player.type} queue`
          );
          return;
        }
        console.log(
          `Player ${player.username} is already in ${player.type} queue, request aborted`
        );
      } else {
  ...
  public removePlayerFromAllQueues(player: Player): void {
    ...
    const pendingIndex = this.pendingPlayers.findIndex(
...
class MatchManager {
  ...
  public createMatch(players: Player[], type: GameMode): Session {
    ...
    console.log(
      `Match created - Type: ${type}, Room: ${uuid_room}, Players: ${players
        .map((p) => p.username)
        .join(", ")}`
    );
    return session;
  ...
  public createTournamentMatch(
    ...
    console.log(
      `Tournament match created - Room: ${roomUuid}, Players: ${players
        .map((p) => p.username)
        .join(", ")}`
    );
    return session;
  ...
  public processMatches(): void {
    ...
    for (let i = joinInvQueue.length - 1; i >= 0; i--) {
      ...
      const creatorIndex = invQueue.findIndex(
        (p) => p.uuid_room === joiner.uuid_room
      );
      ...
      if (creatorIndex !== -1) {
        ...
        const session = this.createMatch(
          [creator, joiner],
          "random_adversaire"
        );
        all_sessions.push(session);
        ...
        console.log(
          `No matching creator found for joiner ${joiner.username} in room ${joiner.uuid_room}`
        );
        secureSend(joiner.socket, {
          type: "error_to_join",
...
class WebSocketMessageHandler {
  ...
  public handleMessage(data: any): void {
    ...
    if (
      data.type === "random_adversaire" ||
      data.type === "local" ||
      data.type === "ia" ||
      data.type === "tournament" ||
      data.type === "inv_game" ||
      data.type === "create_inv_game" ||
      data.type === "join_inv_game" ||
      data.type === "leave_tournament"
    ) {
  ...
  private handleLeavePlayer(_data: any): void {
    ...
    if (this.player.uuid_room) {
      ...
      if (tournament) {
        console.log(
          `Removing player ${this.player.username} from tournament ${this.player.uuid_room}`
        );
        ...
        if (tournament.host.user_id === this.player.user_id) {
          console.log(
            `Host ${this.player.username} is leaving, deleting tournament ${this.player.uuid_room}`
          );
    ...
    if (gameRoom) {
      console.log(
        `Player ${this.player.username} is leaving active game in room ${gameRoom}`
      );

      // Trouver la session de jeu correspondante
      const gameSession = Array.from(sessions.values()).find(
        (session) => session.uuid_room === gameRoom
      );
      ...
      if (gameSession) {
        ...
        if (gameSession.p1 && gameSession.p1.user_id === this.player.user_id) {
          ...
          gameSession.p2 &&
          gameSession.p2.user_id === this.player.user_id
        ) {
    ...
    for (let i = 0; i < createInvQueue.length; i++) {
      if (createInvQueue[i].user_id === this.player.user_id) {
        ...
        console.log(
          `Player ${this.player.username} removed from create invitation queue`
        );
        break;
      }
    }
    ...
    for (let i = 0; i < joinInvQueue.length; i++) {
      if (joinInvQueue[i].user_id === this.player.user_id) {
        joinInvQueue.splice(i, 1);
        console.log(
          `Player ${this.player.username} removed from join invitation queue`
        );
        break;
      }
    }
    ...
    console.log(
      `Player ${this.player.username} has successfully left all games and queues`
    );
  }
  ...
  private handleTournamentPlayerReady(data: any): void {
    ...
    const match = tournament.matches.find(
      (m) =>
        m.status === "pre-active" &&
        (m.player1.user_id === this.player.user_id ||
          m.player2.user_id === this.player.user_id)
    );

    if (!match) {
      console.error(
        `No pre-active match found for player ${this.player.username}`
      );
      return;
    }
    ...
    if (match.readyPlayers.length === 2) {
      ...
      console.log(
        `Player ${this.player.username} ready for match ${match.id}, waiting for opponent`
      );
    }
  }
  ...
  private broadcastTournamentUpdate(tournament: Tournament): void {
    ...
    tournament.players.forEach((player: Player) => {
      if (player.socket && player.socket !== this.socket) {
        console.log(
          `Notifying player ${player.username} about tournament update`
        );
        secureSend(player.socket, {
...
export async function setupMatchmaking(wss: WebSocket) {
  ...
  wss.on("message", function incoming(message: string) {
    try {
      ...
      console.log("matchmaking.ts 742");
      console.log(data);
      ...
      if (
        ...
        playerRooms.delete(wss);
        console.log("delete last status");
      ...
      if (data.uuid_room) {
        ...
        playerRooms.set(wss, data.uuid_room);
      }
      const messageHandler = new WebSocketMessageHandler(wss, connectedPlayer);
      ...
      if (connectedPlayer.uuid_room && connectedPlayer.uuid_room !== "") {
        ...
        playerRooms.set(wss, connectedPlayer.uuid_room);
      }
    } catch (error) {
  ...
  wss.on("close", function () {
    ...
    const storedGameRoom = playerRooms.get(wss);
    handleDisconnection(wss, connectedPlayer, storedGameRoom ?? null);
  });
  ...
  for (const tournament of tournaments) {
    ...
    if (tournament.status === "round1" || tournament.status === "final") {
      ...
      for (const match of pendingMatches) {
        console.log(
          `Starting tournament match: ${match.player1.username} vs ${match.player2.username}`
        );
...
export function handleTournamentMatchEnd(
  ...
  match.player1.socket.send(
    JSON.stringify({
      ...
      opponent:
        match.player1.user_id === winnerId
          ? match.player2.username
          : match.player1.username,
      status: match.status,
    })
  );
  match.player2.socket.send(
    JSON.stringify({
      type: "tournament_match_result",
      tournament_id: tournamentId,
      match_id: match.id,
      ...
      opponent:
        match.player2.user_id === winnerId
          ? match.player1.username
          : match.player2.username,
      status: match.status,
    })
  ...
  if (
    ...
    const winner = tournament.winners.find(
      (w) => w.user_id.toString() === winnerId.toString()
    );
    if (winner) {
  ...
  if (
    roundOneMatches.every((m) => m.status === "completed") &&
    ...
    if (finalMatch) {
      ...
      tournament.winners.forEach((player) => {
        if (player.socket) {
          secureSend(player.socket, {
            ...
            opponent:
              tournament.winners.find((w) => w.user_id !== player.user_id)
                ?.username || "Opponent",
          });
        }
      });
      ...
      console.log(
        `Starting final match: ${tournament.winners[0].username} vs ${tournament.winners[1].username}`
      );
