...
export class TournamentScreen extends BaseScreen {
  ...
  public showCreateTournament(): Promise<string | null> {
    ...
    if (this.container.style.display === 'none') {
      this.container.style.display = 'block';
      const screenRoot = this.container.querySelector('.tournament-screen') as HTMLElement | null;
      if (screenRoot) { screenRoot.style.opacity = '1'; }
    }
    ...
    return new Promise((resolve) => {
      let done = false; const finish = (v:string|null)=>{ if(done) return; done=true; resolve(v); };
  ...
  public showJoinTournament(): Promise<string | null> {
    ...
    return new Promise((resolve) => {
      let done=false; const finish=(v:string|null)=>{ if(done) return; done=true; resolve(v); };
      backBtn.addEventListener('click', () => { finish(null); this.initialCycleActive=false; this.renderRootOptions(); });
      ...
      this.requestTournaments().then(tournaments => {
        if(done) return; if(!tournaments || tournaments.length===0){ subtitle.textContent='No tournaments available'; return; }
      ...
      }).catch(()=>{ if(done) return; subtitle.textContent='Failed to load tournaments'; const retry=document.createElement('button'); retry.className='t-btn primary'; retry.textContent='Retry'; retry.addEventListener('click',()=>{ finish(null); this.showJoinTournament(); }); list.appendChild(retry); });
    });
  }
  ...
  public updateTournamentPlayers(tournamentId: string, players: any[]): void {
      if (this.activeTournamentId === tournamentId && this.lobbyPlayersContainer) {
  ...
  private prepareStartTournament(): void {
      if (this.lobbyPlayersContainer) {
        ...
        const leaveBtn = this.container.querySelector('#lobby-leave') as HTMLButtonElement | null;
        if (leaveBtn) { leaveBtn.disabled = true; leaveBtn.style.opacity = '.5'; }
  ...
  public showTournamentLobby(tournamentId: string, tournamentName: string, initialPlayers: any[], hostId?: string): void {
    this.activeTournamentId = tournamentId; this.initialCycleActive = false;
    const content = this.getContentRoot(); if(!content) return; content.dataset.state='lobby';
    ...
    this.lobbyPlayersContainer = content.querySelector('#lobby-players') as HTMLDivElement;
    this.lobbyStatusEl = content.querySelector('#lobby-status') as HTMLElement;
    this.renderLobbyPlayers(initialPlayers || [], hostId);
    const leaveBtn = content.querySelector('#lobby-leave') as HTMLButtonElement | null;
    leaveBtn?.addEventListener('click', () => {
      if(!this.activeTournamentId) return;
      const ev = new CustomEvent('websocket_request', { detail: { type: 'leave_tournament', tournament_id: this.activeTournamentId }, bubbles: true });
      document.dispatchEvent(ev);
      this.resolving = false; this.activeTournamentId = null; this.lobbyPlayersContainer = null; this.lobbyStatusEl = null; this.renderRootOptions();
    });
  }
  ...
  private renderLobbyPlayers(players: any[], hostId?: string) {
    ...
    players.forEach((player: any, index: number) => {
      ...
      if (hostId && (player.id === hostId || player.user_id === hostId || index === 0)) {
    ...
    if (this.lobbyStatusEl) {
      this.lobbyStatusEl.textContent = total >= 4 ? 'Starting...' : `Waiting for players (${total}/4)`;
    }
  }
}