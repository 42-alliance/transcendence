...
class Player {
  ...
  private compute() {
    ...
    const scale = desiredH / naturalH;
    this.width = Math.round(naturalW * scale);
  ...
  public draw(ctx: CanvasRenderingContext2D) {
    ...
    const label = `${this.width}x${this.height}`;
    ctx.fillText(label, this.x + 4, this.y - 6 < 10 ? this.y + 14 : this.y - 6);
    ctx.restore();
  }
