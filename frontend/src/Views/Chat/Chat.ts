...
async function renderConversations() {
	...
	conversations.forEach(conversation => {
		...
		lastMessageDiv.textContent =  conversation.messages && conversation.messages.length != 0
...
export async function ChatViewListener(conversationId?: number) {
  ...
  const input = document.getElementById("search-new-chat-input") as HTMLInputElement;
